defmodule Pento1Web.GameLive.Picker do
  use Pento1Web, :live_view

  alias Pento1.Game.Board
  import Pento1Web.GameLive.{Colors, Component}
  ...
  def assign_boards(socket) do
    assign(socket,
      :boards,
      Board.puzzles() |> Enum.map(&{&1, Board.new(&1)}) # 게임 크기 리스트-생성함수 assign 추가
    )
  end
  ...
  def row(assigns) do
    ~H"""
      <.link naviate={~p"/game/#{@puzzle}"}>
        <div class="grid gird-cols-2 hover:bg-slate-200">
          <div><h3 class="text-2xl">Piece</h3></div>
          <div>
            <h3 class="text-2xl">
